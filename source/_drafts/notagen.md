...
这里的一个关键点在于，并不是说两级联合训练的思路难以想到，而是这样做其实有代价。
...
这里分析模型指模型的输入数据的熵高于输出数据，生成模型则反过来。
分析模型在于排除噪声干扰，识别数据中存在的某种潜在模式，而生成模型则可不断混入噪声（或从噪声出发），并引向目标分布内的多样性维度。
...
这一点从听感上可能不太明显，但对于基于五线谱的算法作曲方向来说，清晰的声部逻辑正是这里追求的特色之一。
