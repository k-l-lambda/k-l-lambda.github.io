...
这类似于QRCode的容错原理——部分遮挡的二维码仍能被识别出来，道理是一样的。

换个角度，从几何直观来讲，假如绝大多数曲谱对应的μ分布在d维空间中一个低于d维的超平面M上，
高维空间中随即选取的噪声向量ε大概率与超平面M垂直（几乎）。
这样decoder有一个简单策略，把z投影到M上就得到了μ的近似。
当然实际上μ的分布很可能是个形状复杂的流形，但原理上大致类似。
